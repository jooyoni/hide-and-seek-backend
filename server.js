...
io.on('connection', function (socket) {
  socket.on('joinRoom', async (roomNumber) => {
    ...
    socket.on('setLocation', (id, location) => {
      ...
      io.to(roomNumber).emit('updateLocation', id, location.top, location.left);
    });
    ...
    socket.on('attack', (id) => {
      ...
      enemy = enemy.filter((userId) => {
        ...
        const distanceAdvice =
          25 +
          Math.min((Math.min(topDistance, sideDistance) / 90) * 10, 10.3553);
        if (
          Math.sqrt(topDistance ** 2 + sideDistance ** 2) - distanceAdvice <=
          90
        )
          return true;
        else return false;
